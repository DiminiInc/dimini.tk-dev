<!DOCTYPE html>
<html lang="en" style="background-color:#eee">
<head>
<?php include("../global/head.php"); ?>
<title>Hearthstone</title>
<meta name="description" content="Greetings, traveler.">
<link rel="canonical" href="https://legacy.dimini.dev/en/404-kripp">
<link rel="alternate" hreflang="en" href="https://legacy.dimini.dev/en/404-kripp">
<link rel="alternate" hreflang="ru" href="https://legacy.dimini.dev/ru/404-kripp">
<meta property="og:title" content="Hearthstone">
<meta property="og:type" content="website">
<meta property="og:url" content="https://legacy.dimini.dev/en/404-kripp">
<meta property="og:image" content="https://legacy.dimini.dev/site-logo.png">
<meta property="og:description" content="Greetings, traveler.">
</head>
<body>
<?php include("../global/tagmanager.php"); ?>
<div id="error404_content" style="font-size:300%;font-family:Tahoma,Sans-Serif;text-align:center">
<p style="margin-top:0;margin-bottom:20px">
Greetings, traveler.
</p>
<div id="player">
</div>
<script src="https://www.youtube.com/iframe_api"></script>
<script>var player;function onYouTubeIframeAPIReady(){player=new YT.Player("player",{height:"630",width:"1120",videoId:"YXaiJfiut5Y",playerVars:{controls:0,showinfo:0},events:{onReady:onPlayerReady,onStateChange:onPlayerStateChange}})}function onPlayerReady(e){loopStart(),player.playVideo()}function loopStart(){player.seekTo(3)}function onPlayerStateChange(e){e.data==YT.PlayerState.PLAYING&&setTimeout(loopStart,19500)}</script>
</div></body>
</html>